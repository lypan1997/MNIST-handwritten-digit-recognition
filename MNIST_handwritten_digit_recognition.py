...
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 是用CPU还是GPU
EPOCHS = 10  # 训练数据集的轮次

# 3 构建pipeline，对图像做处理
pipeline = transforms.Compose([
    transforms.ToTensor(),  # 将图片转换成tensor
    transforms.Normalize((0.1307,), (0.3081,))  # 正则化作用：降低模型复杂度
])
...
class Digit(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)  # 输入：batch_size*1*28, 输出：batch_size*10*24 (28-5+1=24)
        x = F.relu(x)  # 保持shape不变, 输出：batch_size*10*24
        x = F.max_pool2d(x, 2, 2)  # 池化层：对图片进行压缩（降采样） 输入：batch_size*10*24 输出：batch_size*10*12*12
